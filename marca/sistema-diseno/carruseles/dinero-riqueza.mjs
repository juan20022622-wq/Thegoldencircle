...
const L = [
  `<div class="cabeza">${llave}<h1 class="titular titular--xl" style="margin-top:calc(-24 * var(--u))">La riqueza no se encuentra. <em>Se construye.</em></h1></div><span class="desliza" style="margin-top:auto;margin-bottom:calc(40 * var(--u))">Desliza</span>`,
  `<div class="cabeza">${llave}<h2 class="titular titular--m" style="margin-top:calc(-30 * var(--u))">Un buen hábito encima de otro. <em>Todos los días.</em></h2><p class="cuerpo">Sin prisa y sin atajos. Con método, y con gente que va en la misma dirección.</p></div>`,
  `<div class="cabeza">${llave}<h2 class="titular titular--m" style="margin-top:calc(-30 * var(--u))">El dinero es el primer bloque. <em>Hay cuatro más.</em></h2><p class="cuerpo">Propósito, cuerpo, mentalidad y educación. Con los cinco se levanta algo que dura.</p></div>`,
  `<div class="cabeza"><h2 class="titular titular--m">Cinco llaves. <em>Una puerta que pronto se abre.</em></h2><p class="cuerpo">The Golden Syndicate. Sigue la cuenta para saber cuándo.</p><p class="descargo" style="max-width:31em;color:#A8A293;margin-top:calc(20 * var(--u))">Contenido educativo. No es asesoría financiera. Sin promesas de resultados.</p></div>`,
];
const salida = path.join(aqui, '10-dinero-riqueza'); fs.rmSync(salida, { recursive: true, force: true }); fs.mkdirSync(salida, { recursive: true });
...
for (let i = 0; i < L.length; i++) {
  ...
  fs.writeFileSync(path.join(salida, `lamina-${i + 1}.html`), `<!-- @dsCard group="Carrusel · dinero (riqueza)" name="Lámina ${i + 1}" -->
...
fs.writeFileSync(path.join(salida, 'pie-de-foto.md'), ['# Pie de foto · dinero (riqueza)', '', 'Revisado con `revision-copy-trading`.', '', '---', '', 'La riqueza no se encuentra. Se construye.', '', 'Un buen hábito encima de otro, todos los días. Sin prisa y sin atajos: con método, y con gente que va en la misma dirección.', '', 'El dinero es el primer bloque. Hay cuatro más: propósito, cuerpo, mentalidad y educación. Con los cinco se levanta algo que dura.', '', 'Cinco llaves. Una puerta que pronto se abre. Sigue la cuenta para saber cuándo.', '', '—', 'Contenido educativo. No es asesoría financiera. Sin promesas de resultados.', ''].join('\n'));
